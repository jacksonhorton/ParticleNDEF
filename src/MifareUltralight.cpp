...
MifareUltralight::MifareUltralight(ParticlePN532& nfcShield)
{
    nfc = &nfcShield;
    ndefStartIndex = 0;
    messageLength = 0;
...
NfcTag MifareUltralight::read(byte * uid, unsigned int uidLength)
{
    ...
    {
#ifdef NDEF_LOGGING
        Log.warn("WARNING: Tag is not formatted.");
#endif
...
boolean MifareUltralight::isUnformatted()
{
    ...
    {
#ifdef NDEF_LOGGING
        Log.error("Error. Failed read page %d", page);
#endif
        return false;
    }
}
...
boolean MifareUltralight::write(NdefMessage& m, byte * uid, unsigned int uidLength)
{
    ...
    if(bufferSize>tagCapacity) {
	    #ifdef MIFARE_ULTRALIGHT_DEBUG
    	Log.error("Encoded Message length exceeded tag Capacity %d", tagCapacity);
    	#endif
    	return false;
    }
